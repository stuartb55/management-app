...
// Update import for calculateNextDueDate
import { calculateNextDueDate } from "@/lib/utils"

type TaskManagementProps = {
  initialTasks: Task[]
  staff: Staff[]
  currentStaffId?: string // Optional, if managing tasks for a specific staff member
}
  ...
  const handleAddOrUpdateTask = async () => {
    ...
    if (isRecurringDialogOpen && newTask.dueDate) {
      const pattern: RecurringPattern = {
        ...recurringFormData,
        startDate: recurringFormData.startDate || new Date(newTask.dueDate),
        currentOccurrence: 1,
      }
      taskToSubmit.recurringPattern = pattern
      taskToSubmit.nextDueDate = calculateNextDueDate(new Date(newTask.dueDate), pattern)
    } else {
    ...
    <div className="grid gap-6">
            ...
            <Popover open={isRecurringDialogOpen} onOpenChange={setIsRecurringDialogOpen}>
              <PopoverTrigger asChild>
                <Button variant="outline" size="sm">
                  {newTask.recurringPattern ? "Edit Recurrence" : "Set Recurrence"}
                </Button>
              </PopoverTrigger>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>All tasks assigned to this staff member.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4">
          {tasks.length === 0 ? (
