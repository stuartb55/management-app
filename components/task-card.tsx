...
import { calculateNextDueDate } from "@/lib/utils" // Import from database.ts
import { DatePicker } from "@/components/date-picker"
// Update import for formatDate
...
export function TaskCard({ task, staff, onTaskUpdated, onTaskDeleted, onTaskAdded }: TaskCardProps) {
  ...
  const handleRecurringFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    const { name, value } = e.target
    ...
    <Card className="w-full">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <div className="flex items-center gap-2">
          ...
          {isEditing ? (
            <Input name="title" value={editedTask.title} onChange={handleInputChange} className="text-lg font-bold" />
          ) : (
        ...
        {editedTask.recurringPattern && (
          <div className="mt-4 p-3 border rounded-md bg-muted/50">
            ...
            <p className="text-xs text-muted-foreground">
              Frequency: Every {editedTask.recurringPattern.interval} {editedTask.recurringPattern.frequency}
            </p>
            {editedTask.nextDueDate && (
              <p className="text-xs text-muted-foreground">Next Instance Due: {formatDate(editedTask.nextDueDate)}</p>
            )}
                ...
                <PopoverContent className="w-80">
                  <div className="grid gap-4">
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-muted-foreground">Define how often this task repeats.</p>
                    </div>
                    <div className="grid gap-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          setRecurringFormData((prev) => ({ ...prev, frequency: value as RecurringFrequency }))
                        }
                      >
                        <SelectContent>
